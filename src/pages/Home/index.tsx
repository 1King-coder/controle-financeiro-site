...
import { Container, OptionBtn, ScrollableDiv, ScrollableDivY, StyledButton, StyledButtonGroup } from "../../styles/GlobalStyles";
  ...
  static async getGastosGeraisFilterByMonthInterval (date: Date, id_user: Number): Promise<GastoGeral[]> {
    const filteredData =  GetGastosGeraisDataFuncions.getGastosGerais(id_user).then((data: GastoGeral[]) => {
      ...
      const filteredData = data.filter( (gastoGeral: GastoGeral) => {
        ...
        if (new Date(gastoGeral.data_de_competencia).getMonth() === date.getMonth()) {
          return gastoGeral;
        }
...
export default function Home(): JSX.Element {
  ...
  const [categoriasByNameId, setCategoriasByNameId]: [any, any] = React.useState([]);
  ...
  React.useEffect(() => { 
    if (user?.isAuthenticated) {
      GetGastosGeraisDataFuncions.getGastosGeraisFilterByMonthInterval(new Date(), user!.id).then((data: GastoGeral[]) => {
              ...
              <>
                <ScrollableDivBtns>
                  <Btn onClick={() => history.push("/gastos-gerais/add")}>
                    ...
                    <FaMoneyBillTransfer size={18} color={secondaryColor} style={{margin: "0 0 0 10px"}}/>
                  </Btn>
                  <Btn onClick={() => history.push("/depositos/add")}>
                    <span>Adicionar Depósito</span>
                    <GiMoneyStack size={18} color={secondaryColor} style={{margin: "0 0 0 10px"}}/>
                  </Btn>
                  <Btn onClick={() => history.push("/transferencias/add")}>
                    <span>Adicionar Transferência</span>
                    <BiTransfer size={18} color={secondaryColor} style={{margin: "0 0 0 10px"}}/>
                  </Btn>
                </ScrollableDivBtns>
                <Box>
                  <Link to={`/bancos`}>
                ...
                <Box>
                  ...
                  <StyledButton style={{fontSize: 20, height: "75px"}} onClick={() => history.push("/categorias/add")}>Adicionar nova Categoria</StyledButton>
                </Box>
                <Box>
