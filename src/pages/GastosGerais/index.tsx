...
import { dayOfTheWeek, fixDate, months } from "../../config/dates";
...
class GetGastosGeraisDataFuncions {
  ...
  static async getGastosGeraisFilterByDateInterval(
    dateStart: Date,
    dateEnd: Date,
    id_user: Number,
  ): Promise<GastoGeral[]> {
    const filteredData = GetGastosGeraisDataFuncions.getGastosGerais(
      ...
      const filteredData = data.filter((gastoGeral: GastoGeral) => {
        // @ts-ignore
        const dateSplited = fixDate(
          new Date(gastoGeral.data_de_competencia).toLocaleDateString("pt-br", {
  ...
  static async getGastosGeraisFilterByLastWeek(
    id_user: Number,
  ): Promise<GastoGeral[]> {
    const dateStart = new Date();
    const dateEnd = new Date();
    dateStart.setDate(dateStart.getDate() - 7);
    return await GetGastosGeraisDataFuncions.getGastosGeraisFilterByDateInterval(
      dateStart,
      dateEnd,
      id_user,
    );
...
export function GastosGerais(): JSX.Element {
  ...
  const [optionSelectedId, setOptionSelectedId]: [number, any] =
    React.useState(2);
  const [gastosByActualWeekDay, setgastosByActualWeekDay] = React.useState(
    Array<GastoGeral[]>([]),
  );
  ...
  React.useEffect(() => {
    ...
    GetGastosGeraisDataFuncions.getGastosGeraisFilterByDateInterval(
      semanaSelecionada[0],
      semanaSelecionada[1],
      user!.id,
    ).then((data: GastoGeral[]) => {
      ...
      <div>
        {optionSelectedId === 1 ? (
          <div style={{ marginTop: "2rem" }}>
            ...
            >
              {gastosByActualWeekDay.map(
                (gastosInWeekDay: GastoGeral[], index: number) => {
                  return (
                    <WeekDayPopoverCard
                      itemUrlPath="gastos-gerais"
                      key={index}
                      bancos={bancos}
                      categorias={categorias}
                      itemInWeekDay={gastosInWeekDay}
                      index={index}
                    />
                  );
                },
          ...
          >
            ...
            {(() => {
              ...
              const totalGastos = gastosByMonth.reduce(
