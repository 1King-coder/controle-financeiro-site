...
import {
  SheetNames,
  EntradasSaidasColumns,
  TransferenciasColumns,
} from "../../config/ssChkConstants";
...
function checkSpreadSheetFileModel(workbook: WorkBook): boolean {
  if (
    !(
      workbook.SheetNames.includes(SheetNames[0]) &&
      workbook.SheetNames.includes(SheetNames[1])
    )
  ) {
    toast.error(
      "O modelo da planilha está incorreto. Verifique o modelo e tente novamente.",
    );
    return false;
  }

  const esSheetHeaders = utils.sheet_to_json(workbook.Sheets[SheetNames[0]], {
    header: 1,
  })[0] as Array<string>;
  ...
  const trSheetHeaders = utils.sheet_to_json(workbook.Sheets[SheetNames[1]], {
    header: 1,
  })[0] as Array<string>;
  ...
  for (let i = 0; i < esSheet.length; i++) {
    ...
    if (
      esSheet[i]["Data de competência"] &&
      isNaN(Date.parse(esSheet[i]["Data de competência"]))
    ) {
      toast.error(
        "O modelo da planilha está incorreto. Verifique o modelo e tente novamente.",
      );
  ...
  for (let i = 0; i < trSheet.length; i++) {
    ...
    if (
      trSheet[i]["Tipo de transferência"] &&
      !(
        trSheet[i]["Tipo de transferência"] === "Entrada" ||
        trSheet[i]["Tipo de transferência"] === "Saída"
      )
    ) {
      toast.error(
        "O modelo da planilha está incorreto. Verifique o modelo e tente novamente.",
    ...
    if (
      trSheet[i]["Origem"] &&
      trSheet[i]["Destino"] &&
      trSheet[i]["Origem"] === trSheet[i]["Destino"]
    ) {
      toast.error(
        "O modelo da planilha está incorreto. Verifique o modelo e tente novamente.",
      );
    ...
    if (
      trSheet[i]["Data de competência"] &&
      isNaN(Date.parse(trSheet[i]["Data de competência"]))
    ) {
      toast.error(
        "O modelo da planilha está incorreto. Verifique o modelo e tente novamente.",
      );
...
export default function EnviaPorPlanilha() {
  ...
  React.useEffect(() => {
    GetBancosDataFuncions.getBancos(user!.id).then((data: Banco[]) => {
      ...
      setIdBancos(
        Object.assign({}, ...data.map((banco) => ({ [banco.nome]: banco.id }))),
      );
    });
  }, [user]);
    ...
    GetCategoriasDataFuncions.getCategorias(user!.id).then(
      (data: Categoria[]) => {
        ...
        setIdCategorias(
          Object.assign(
            {},
            ...data.map((categoria) => ({ [categoria.nome]: categoria.id })),
          ),
        );
      },
    );
  ...
  function handleSelectFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (inputElement && inputElement.files && inputElement.files.length > 0) {
      document.querySelector<HTMLElement>("#file-name")!.innerText =
  ...
  function loadPlanilha(WorkBook: WorkBook) {
    if (checkSpreadSheetFileModel(WorkBook)) {
      ...
      const esSheet = utils.sheet_to_json(WorkBook.Sheets[SheetNames[0]], {
        header: 1,
      }) as Array<Array<any>>;
      let esData = esSheet
        .slice(1)
        .map((row, index) => ({
          ...
          descricao: row[2] || "",
          valor: row[3] || 0,
          dataCompetencia: row[4]
            ? new Date(
                new Date(getJsDateFromExcel(row[4])).setDate(
                  new Date(getJsDateFromExcel(row[4])).getDate() + 1,
                ),
              )
      ...
      const trSheet = utils.sheet_to_json(WorkBook.Sheets[SheetNames[1]], {
        header: 1,
      }) as Array<Array<any>>;
      let trData = trSheet
        .slice(1)
        .map((row, index) => ({
          ...
          destino: row[4] || "",
          intermediario: row[5] || "",
          dataCompetencia: row[6]
            ? new Date(
                new Date(getJsDateFromExcel(row[6])).setDate(
                  new Date(getJsDateFromExcel(row[6])).getDate() + 1,
                ),
              )
                .toLocaleDateString("pt-br")
                .split("/")
      ...
      trData.forEach((tr) => {
        if (tr.tipoTransferencia === "Entre Bancos") {
          ...
          if (
            !bancosNames.includes(tr.destino) ||
            !bancosNames.includes(tr.origem)
          ) {
            toast.error(
              `Banco de origem ou destino inválido na transferência: ${tr.descricao}`,
            );
          }
          ...
          if (
            !categoriasNames.includes(tr.destino) ||
            !categoriasNames.includes(tr.origem)
          ) {
            toast.error(
              `Categoria de origem ou destino inválida na transferência: ${tr.descricao}`,
            );
          }
        }
      });
      ...
      trData = trData.filter((tr) => {
        if (tr.tipoTransferencia === "Entre Bancos") {
          return (
            bancosNames.includes(tr.origem) &&
            bancosNames.includes(tr.destino) &&
            categoriasNames.includes(tr.intermediario)
          );
        } else if (tr.tipoTransferencia === "Entre Categorias") {
          return (
            categoriasNames.includes(tr.origem) &&
            categoriasNames.includes(tr.destino) &&
            bancosNames.includes(tr.intermediario)
          );
        }
      });
      esData = esData.filter((es) => {
        return (
          bancosNames.includes(es.banco) &&
          categoriasNames.includes(es.categoria)
        );
      });
    ...
    for (let i = 0; i < transferenciasData.length; i++) {
      if (transferenciasData[i]["tipoTransferencia"] === "Entre Bancos") {
        if (
          ...
          toast.error(
            `Erro ao enviar transferência: ${transferenciasData[i].descricao}. Verifique os dados e tente novamente.`,
          );
          return;
        }
        ...
        transferenciasData[i]["tipoTransferencia"] === "Entre Categorias"
      ) {
        if (
  ...
  async function saveEntradasSaidas() {
    ...
    for (let i = 0; i < entradasSaidasData.length; i++) {
      if (
        ...
        toast.error(
          `Erro ao enviar entrada/saída: ${entradasSaidasData[i].descricao}. Verifique os dados e tente novamente.`,
        );
        return;
      }
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Erro ao enviar entradas/saídas",
      );
    }
  }
